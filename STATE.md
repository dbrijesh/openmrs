...
## Current Status: SPRING BOOT MIGRATION IN PROGRESS
...
- [x] spring-boot-starter-parent 3.4.5 added as root parent
- [x] webapp/pom.xml: spring-boot-starter-web, actuator, tomcat(provided), spring-boot-maven-plugin
- [x] webapp/src/main/java/org/openmrs/web/OpenmrsApplication.java — @SpringBootApplication entry point
- [x] webapp/src/main/java/org/openmrs/web/WebConfig.java — Java config replacing web.xml (filters + servlets)
- [x] webapp/src/main/resources/application.properties — server.port=8080, context-path=/openmrs
- [x] Fixed: removed undefined ${springVersion} spring-framework-bom import from root pom
- [x] Fixed: added missing version properties (hibernateVersion, junitVersion, log4jVersion, slf4jVersion, jacksonVersion, hamcrestVersion, mockitoVersion)
- [x] **mvn package -DskipTests -P skip-all-checks: ALL 10 MODULES BUILD SUCCESS in 1:08 min**
- [x] openmrs.war (122 MB) produced with spring-boot-3.4.5.jar + OpenmrsApplication.class
...
mvn package -DskipTests -P skip-all-checks
# Then run:
java -jar webapp/target/openmrs.war
```

## Pending
- [ ] Runtime smoke test: start openmrs.war and verify /openmrs/actuator/health responds
