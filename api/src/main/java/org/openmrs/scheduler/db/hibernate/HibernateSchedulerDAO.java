...
public class HibernateSchedulerDAO implements SchedulerDAO {
	...
	public TaskDefinition getTaskByName(String name) throws DAOException {
		Query<TaskDefinition> query = sessionFactory.getCurrentSession()
			.createQuery("FROM TaskDefinition t WHERE t.name = :name", TaskDefinition.class)
			.setParameter("name", name);
		TaskDefinition task = query.uniqueResult();
	...
	public List<TaskDefinition> getTasks() throws DAOException {
		return sessionFactory.getCurrentSession().createQuery("FROM TaskDefinition", TaskDefinition.class).list();
	}
	...
	public TaskDefinition getTaskByUuid(String uuid) throws DAOException {
		return (TaskDefinition) sessionFactory.getCurrentSession()
				.createQuery("from TaskDefinition o where o.uuid = :uuid", TaskDefinition.class).setParameter("uuid", uuid).uniqueResult();
	}
}
