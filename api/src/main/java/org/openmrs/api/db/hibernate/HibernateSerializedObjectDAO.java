...
public class HibernateSerializedObjectDAO implements SerializedObjectDAO {
	...
	public SerializedObject getSerializedObjectByUuid(String uuid) throws DAOException {
		...
		if (uuid != null) {
			ret = sessionFactory.getCurrentSession()
			        .createQuery("from SerializedObject s where s.uuid = :uuid", SerializedObject.class)
	...
	public List<SerializedObject> getAllSerializedObjectsByName(Class<?> type, String name, boolean exactMatchOnly)
		...
		StringBuilder hql = new StringBuilder(
		    "from SerializedObject s where (s.type = :typeName or s.subtype = :typeName) and ");
		if (exactMatchOnly) {
			hql.append("s.name = :name");
		} else {
			hql.append("lower(s.name) like :name");
		}
		Query<SerializedObject> query = sessionFactory.getCurrentSession()
		        .createQuery(hql.toString(), SerializedObject.class);
	...
	public List<SerializedObject> getAllSerializedObjects(Class<?> type, boolean includeRetired) throws DAOException {
		StringBuilder hql = new StringBuilder(
		    "from SerializedObject s where (s.type = :typeName or s.subtype = :typeName)");
		if (!includeRetired) {
