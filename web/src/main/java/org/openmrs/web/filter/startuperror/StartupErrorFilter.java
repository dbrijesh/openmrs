	...
	protected void doPost(HttpServletRequest httpRequest, HttpServletResponse httpResponse)
		...
		if (getUpdateFilterModel().errorAtStartup instanceof OpenmrsCoreModuleException) {
			...
			try {
				...
				for (Part part : parts) {
					if (part.getContentType() != null && !part.getContentType().isEmpty()) {
						try (InputStream uploadedStream = part.getInputStream()) {
							ModuleUtil.insertModuleFile(uploadedStream, part.getSubmittedFileName());
						}
					}
				}
			}
