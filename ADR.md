...
## ADR-011: Migrate to Spring Boot 3.4.5 (Not Spring Boot 4.x)
**Date**: 2026-06-02
**Status**: Accepted

### Context
User requested migration to Spring Boot 4.*. Spring Boot 4.0 is not yet GA (planned late 2025/2026).
Spring Boot 3.4.5 is the current stable release based on Spring Framework 6.x (our exact version).

### Decision
Migrate to Spring Boot 3.4.5.

### Rationale
- Spring Boot 4.0 is not GA; only milestones/snapshots available
- Spring Boot 3.4.5 uses Spring Framework 6.2.x (identical to what we have)
- All dependency versions we use are managed by Spring Boot 3.4.5 BOM
- Path to Spring Boot 4.0 is trivial after 3.x: change parent version + fix Spring 7 APIs

---

## ADR-012: Keep WAR Packaging for Spring Boot (Not Executable JAR)
**Date**: 2026-06-02
**Status**: Accepted

### Context
Spring Boot defaults to executable JAR. OpenMRS has a module system that loads `.omod` files
as plugins using a custom ClassLoader. This requires a standard servlet container classloader
hierarchy that executable JAR packaging disrupts.

### Decision
Use WAR packaging with embedded Tomcat (spring-boot-starter-tomcat as `provided`).

### Rationale
- OpenMRS module classloading requires the standard webapp ClassLoader hierarchy
- WAR can still embed Tomcat AND run as standalone (`java -jar`) via SpringBootServletInitializer
- WAR packaging allows future deployment to external Tomcat if needed

---

## ADR-013: Keep XML Spring Configuration via @ImportResource
**Date**: 2026-06-02
**Status**: Accepted

### Context
OpenMRS has 600+ lines of XML Spring bean definitions in applicationContext-service.xml.
Migrating all to Java @Configuration would be a huge change unrelated to Spring Boot migration.

### Decision
Load existing XML configs via `@ImportResource` in the @SpringBootApplication class.

### Rationale
- Preserves existing, battle-tested configuration
- Spring Boot supports @ImportResource natively
- Reduces migration risk — only infrastructure changes, no bean logic changes
- XML → Java config can be done incrementally in future PRs

---

## ADR-014: Exclude Spring Boot DataSource and JPA Autoconfiguration
**Date**: 2026-06-02
**Status**: Accepted

### Context
OpenMRS manages its own Hibernate SessionFactory via HibernateSessionFactoryBean
and owns its own DataSource via C3P0 connection pool (configured in hibernate properties).
Spring Boot's DataSource/JPA autoconfiguration would conflict.

### Decision
Exclude `DataSourceAutoConfiguration`, `HibernateJpaAutoConfiguration`, and
`DataSourceTransactionManagerAutoConfiguration` from Spring Boot autoconfiguration.

### Rationale
- OpenMRS reads datasource config from runtime properties file (not application.properties)
- OpenMRS uses SessionFactory (not EntityManager) — not compatible with JPA autoconfiguration
- The custom HibernateSessionFactoryBean handles all Hibernate lifecycle
